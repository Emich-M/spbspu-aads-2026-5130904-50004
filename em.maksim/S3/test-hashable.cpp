...
BOOST_AUTO_TEST_CASE(default_construction) {
  em::HashTable<int, int, std::hash<int>, std::equal_to<int>> ht;
...
BOOST_AUTO_TEST_CASE(at_access) {
  em::HashTable<std::string, int, std::hash<std::string>, std::equal_to<std::string>> ht;
  ht.add("apple", 5);
  ht.add("banana", 7);
...
BOOST_AUTO_TEST_CASE(clear_table) {
  em::HashTable<char, char, std::hash<char>, std::equal_to<char>> ht;
  ht.add('a', 'A');
  ht.add('b', 'B');
  BOOST_TEST(ht.size() == 2);
  ht.clear();
...
BOOST_AUTO_TEST_CASE(const_iterator_basic) {
  em::HashTable<double, int, std::hash<double>, std::equal_to<double>> ht;
  ht.add(1.1, 100);
  ht.add(2.2, 200);
  const auto& cht = ht;
